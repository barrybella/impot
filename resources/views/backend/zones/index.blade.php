...
@endif
    <div class="panel">
        <div class="panel-heading">
            <h3 class="panel-title" style="display: inline;">Liste des Zones</h3>
            @can('create', App\Zones::class)
                  ...
                  @foreach($zones as $cd)
                      <tr>
                        ...
                        <!-- <td>{{Date::make($cd->created_at)->format('d F Y')}}</td> -->
                          <td>
                              <form method="POST" action="{{ route('zones.destroy', ['id' => $cd->id]) }}"
                                    onsubmit="return confirm('Voulez-vous vraiment supprimé cet zones')">
                                  <input type="hidden" name="_method" value="DELETE">
                                  <input type="hidden" name="_token" value="{{ csrf_token() }}">
                                  @can('edit', App\Zones::class)
                                      <a href="{{route('zones.edit', ['id' => $cd->id]) }}" class="btn btn-warning btn-icon btn-circle"
                                         class="btn btn-warning btn-xs">
                                          <i class="glyphicon glyphicon-edit"></i>
                                      </a>
                                  @endcan
                                  @can('delete', App\Zones::class)
                                  <button type="submit" class="btn btn-danger btn-icon btn-circle">
                                      <i class="glyphicon glyphicon-trash"></i>
                                  </button>
                                  @endcan
                                  @can('show', App\Zones::class)
                               <a href="{{ route('zones.show', $cd->id) }}"
